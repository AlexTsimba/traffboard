  ...
  default: vi.fn(() => {
    const queryFn = Object.assign(
      vi
        .fn()
        .mockResolvedValue([
          { health_check: 1, version: "PostgreSQL 14.0", current_database: "test", server_ip: "127.0.0.1" },
        ]),
      {
...
describe("Database Connection", () => {
  ...
  describe("exports", () => {
    ...
    it("should export enhanced utility functions", async () => {
      const { checkDatabaseHealth, closeDatabaseConnection, getDatabaseInfo, executeWithRetry, getConnectionMetrics } =
...
describe("Enhanced Database Connection Features", () => {
  ...
  describe("retry mechanism", () => {
    ...
    it("should retry failed operations", async () => {
      ...
      const mockOperation = vi.fn().mockRejectedValueOnce(new Error("First failure")).mockResolvedValue("success");
