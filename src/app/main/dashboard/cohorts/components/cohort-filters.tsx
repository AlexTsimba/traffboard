...
export function CohortFilters({ currentFilters }: CohortFiltersProps) {
  ...
  const appliedFilters = useMemo((): AppliedFilter[] => {
    ...
    if (cohortStep && cohortStep !== 'day') { // показываем только если не дефолт
    ...
    if (metric && metric !== 'retention') { // показываем только если не дефолт
  ...
  const handleApplyFilters = (
    ...
    Object.entries(generalFilters).forEach(([key, value]) => {
      if (key === 'dateRange' && value && typeof value === 'object') {
        const dateRange = value as { start: string | Date; end: string | Date };
        const startISO = typeof dateRange.start === 'string' 
          ? dateRange.start 
          : dateToISOString(dateRange.start);
        const endISO = typeof dateRange.end === 'string' 
          ? dateRange.end 
          : dateToISOString(dateRange.end);
